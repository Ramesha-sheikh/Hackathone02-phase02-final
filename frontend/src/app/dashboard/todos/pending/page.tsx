...
export default function PendingTasksPage() {
  ...
  const fetchTodos = async () => {
    ...
    try {
      ...
      // Filter for pending tasks (not completed and no due date or future due date)
      const now = new Date();
      const pending = data.filter(todo =>
        !todo.completed &&
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 md:mb-8">
        <h1 className="text-3xl md:text-4xl font-bold text-yellow-400 mb-4 md:mb-0 flex items-center">
      ...
      {/* Pending Tasks List */}
      <div className="bg-[color:var(--bg-card)] rounded-lg border border-yellow-500/50 shadow-[0_0_15px_rgba(255,255,0,0.2)] p-4 md:p-6 w-full">
        {todos.length === 0 ? (
          <div className="text-center py-12">
            <p className="text-xl text-[color:var(--text-secondary)] mb-4">No pending tasks. All caught up!</p>
          ...
          <div className="space-y-4">
            {todos.map((todo) => (
              <div
                key={todo.id}
                className="flex flex-col md:flex-row items-start md:items-center justify-between bg-yellow-500/10 p-4 rounded-lg border border-yellow-500/50 hover:shadow-[0_0_15px_rgba(255,255,0,0.3)] transition-all duration-300 transform hover:scale-[1.01]"
              >
